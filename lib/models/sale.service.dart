...
class SaleService implements Sale {
  @override
  String? id;
  @override
  String? clientId;
  ...
  Map<String, dynamic> toDisplay() {
    return {
      'RNC/Cédula o Pasaporte': clientId?.replaceAll('-', ''),
  ...
  factory SaleService.fromMap(Map<String, dynamic> map) {
    return SaleService(
        id: map['id'],
  ...
  factory SaleService.fromJson(String source) =>
      SaleService.fromMap(json.decode(source) as Map<String, dynamic>);
  ...
  @override
  Future<SaleService?> create() async {
    var seqParams = '';
    ...
    try {
      ...
      '''), parameters: map);
        ...
        for (int i = 0; i < items.length; i++) {
          ...
          subMap['id'] = xid;
          subMap['saleId'] = id;
          await conne.execute(Sql.named('''INSERT INTO public."SaleService"(
	       id, "serviceId", discount, net, tax, total, "retentionTax", "retentionIsr", "saleId", quantity, "taxId", "discountId", "retentionTaxId", "retentionIsrId",tax18,tax16,tax3,net18,net16,net3,"exemptAmount","indicadorFacturacion","indicadorAgentePercepcion","serviceName")
	       VALUES (@id, @serviceId, @discount, @net, @tax, @total, @retentionTax, @retentionIsr, @saleId, @quantity, @taxId, @discountId, @retentionTaxId, @retentionIsrId,@tax18,@tax16,@tax3,@net18,@net16,@net3,@exemptAmount,@indicadorFacturacion,@indicadorAgentePercepcion,@serviceName); '''),
              parameters: subMap);
        }
  ...
  Future<Sale?> delete() async {
    try {
      ...
      await conn?.runTx((c) async {
        await c.execute(
            Sql.named('DELETE FROM public."Payments" WHERE "saleId" = @id'),
            parameters: {'id': id});
  ...
  @override
  Future<SaleService?> update() async {
    try {
      final conne = SqlConector.connection;
      await conne?.runTx((conne) async {
  ...
  Future<Sale?> paySale(double amount) async {
    try {
      ...
      var el = res2?[0];
      if (el != null) {
        return SaleService.fromMap(el.toColumnMap());
      }
      return null;
    } catch (e) {
      rethrow;
